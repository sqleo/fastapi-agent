...
from llamarag.jobs import run_kb_file_index_job, run_kb_file_parse_job
...
@broker.task
async def kb_file_index_task(kb_id: int, file_id: int, owner_user_id: int) -> None:
    """Worker 内执行 ``run_kb_file_index_job``（入库向量库）。"""
    async with async_session() as session:
        await run_kb_file_index_job(
            session,
            kb_id=kb_id,
            file_id=file_id,

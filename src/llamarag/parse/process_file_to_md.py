...
from llamarag.utils.content_semver import bump_patch_after_parse
...
def parsed_md_storage_key(owner_user_id: int, file_id: int) -> str:
    """相对 ``static/`` 的中间 Markdown 路径（与库表约定一致）."""
    return f"{PARSED_MD_STORAGE_PREFIX}/{owner_user_id}/{file_id}.md"


async def process_file_to_md(
    session: AsyncSession,
    *,
    owner_user_id: int,
    file_id: int,
) -> FileAssetModel:
    """校验归属后生成中间 Markdown，更新文件记录与关联知识库条目."""
    ...
    key = parsed_md_storage_key(owner_user_id, file_id)
    ...
    kb_stmt = select(KnowledgeBaseFileModel).where(
        col(KnowledgeBaseFileModel.owner_user_id) == owner_user_id,
        col(KnowledgeBaseFileModel.file_id) == file_id,
    )
